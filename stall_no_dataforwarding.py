    ...
    while j<len(parameter):
        ...
        if parameter[j]==0:
            ...
            #print(reg)
                ...
                if (pipeline[j-1][0]== pipeline[j][1] and pipeline[j][1]!=-1 and pipeline[j-1][0]!=0) or (pipeline[j-1][0] == pipeline[j][2] and pipeline[j][2]!=-1 and pipeline[j-1][0]!=0):
                    flag=1;
                    stalls_data_hazard = stalls_data_hazard + 1
                    if parameter[j-1]==3:
                        data_hazard=data_hazard+1	
                    break;
                elif ( (j-2)>=0 and pipeline[j-2][0]==pipeline[j][1] and pipeline[j][1]!=-1 and pipeline[j-2][0]!=0 ) or ( (j-2)>=0 and pipeline[j-2][0]==pipeline[j][2] and pipeline[j][2]!=-1 and pipeline[j-2][0]!=0 ):
                    flag=1;
                    stalls_data_hazard = stalls_data_hazard + 1
                    if parameter[j-2]==4:
                        data_hazard=data_hazard+1
            ...
            parameter[j]=3
            if (pipeline[j][6]==1 and pipeline[j][4]=='SB') or (pipeline[j][4]=='UJ') or(pipeline[j][4]=='I_3'):
                change=0
                flag1=1
